...
final class SSHKeyExchangeStateMachineTests: XCTestCase {
    ...
    func testExtraECDHReplyForbidden() throws {
        ...
        let message = SSHMessage.keyExchangeReply(.init(hostKey: .init(backingKey: privateKey.publicKey), publicKey: ByteBufferAllocator().buffer(capacity: 1024), signature: .init(backingSignature: .ed25519(Curve25519Signature(rawRepresentation: Data(repeating: 0x00, count: 1024))))))
