...
private enum Fixtures {
    ...
    static let caPublicKey =
        "ecdsa-sha2-nistp384 AAAAE2VjZHNhLXNoYTItbmlzdHAzODQAAAAIbmlzdHAzODQAAABhBHYlMSXacXt13oBLpMXEP0OSMw5okd5c7G3hoim1MR/THUOyOS2AVQKEqLZs+td3Y6yYCrq5TGWDNGY2dfKFX99nLqJCq2kxR//CP3UherkZnn6u4eW4biLL7xODqNOzkQ== lukasa@MacBook-Pro.local"
    static let caPublicKeyExport =
        "ecdsa-sha2-nistp384 AAAAE2VjZHNhLXNoYTItbmlzdHAzODQAAAAIbmlzdHAzODQAAABhBHYlMSXacXt13oBLpMXEP0OSMw5okd5c7G3hoim1MR/THUOyOS2AVQKEqLZs+td3Y6yYCrq5TGWDNGY2dfKFX99nLqJCq2kxR//CP3UherkZnn6u4eW4biLL7xODqNOzkQ=="
    ...
    static let p256UserBaseExport =
        "ecdsa-sha2-nistp256 AAAAE2VjZHNhLXNoYTItbmlzdHAyNTYAAAAIbmlzdHAyNTYAAABBBIZS1APJofiPeoATC/VC4kKi7xRPdz934nSkFLTc0whYi3A8hEKHAOX9edgL1UWxRqRGQZq2wvvAIjAO9kCeiQA="
    ...
    static let p384HostBaseExport =
        "ecdsa-sha2-nistp384 AAAAE2VjZHNhLXNoYTItbmlzdHAzODQAAAAIbmlzdHAzODQAAABhBJPOgAXHijSxoZBiyhSDOR3eUELUoc+hqh/SY1Wq4/562jThf6Q+tjVzZTMWZMAP4S6DD2qZswsRvisxXkcZDOw5bvyk0WmezYvjUP6TZII/0BDVTotCf4SxukEtcqBZqg=="
...
final class CertifiedKeyTests: XCTestCase {
    ...
    func testStraightForwardHappyPathVerificationHost() throws {
        ...
        // Let's try both valid principals
        var criticalOptions = try hostKey.validate(
            principal: "localhost",
            type: .host,
        ...
        criticalOptions = try hostKey.validate(
    ...
    func testStraightForwardHappyPathVerificationUserEd25519() throws {
        ...
        var criticalOptions = try userKey.validate(
            principal: "foo",
            type: .user,
            allowedAuthoritySigningKeys: [caKey],
        ...
        criticalOptions = try userKey.validate(
            principal: "bar",
            type: .user,
            allowedAuthoritySigningKeys: [caKey],
            acceptableCriticalOptions: ["force-command"]
    ...
    func testKeysThatDontMatchPrincipalAreRejected() throws {
        ...
        let userKey = try NIOSSHCertifiedPublicKey(NIOSSHPublicKey(openSSHPublicKey: Fixtures.p256User))!
        let hostKey = try NIOSSHCertifiedPublicKey(NIOSSHPublicKey(openSSHPublicKey: Fixtures.p384Host))!
        XCTAssertThrowsError(try userKey.validate(principal: "baz", type: .user, allowedAuthoritySigningKeys: [caKey]))
        { error in
    ...
    func testKeysThatDontMatchTypeAreRejected() throws {
        ...
        XCTAssertThrowsError(try userKey.validate(principal: "foo", type: .host, allowedAuthoritySigningKeys: [caKey]))
        { error in
            XCTAssertEqual((error as? NIOSSHError)?.type, .invalidCertificate)
        }
        XCTAssertThrowsError(
        ...
        XCTAssertThrowsError(try edKey.validate(principal: "foo", type: .host, allowedAuthoritySigningKeys: [caKey])) {
            error in
            XCTAssertEqual((error as? NIOSSHError)?.type, .invalidCertificate)
        }
    }
    ...
    func testKeysSignedByTheWrongEntityAreRejected() throws {
        ...
        let edKey = try NIOSSHCertifiedPublicKey(NIOSSHPublicKey(openSSHPublicKey: Fixtures.ed25519User))!
        XCTAssertThrowsError(
            try userKey.validate(principal: "foo", type: .user, allowedAuthoritySigningKeys: [badCAKey])
        ) { error in
            XCTAssertEqual((error as? NIOSSHError)?.type, .invalidCertificate)
        }
        XCTAssertThrowsError(
            try hostKey.validate(
                principal: "localhost",
                type: .host,
                allowedAuthoritySigningKeys: [badCAKey],
                acceptableCriticalOptions: ["cats"]
            )
        ) { error in
            XCTAssertEqual((error as? NIOSSHError)?.type, .invalidCertificate)
        }
        XCTAssertThrowsError(try edKey.validate(principal: "foo", type: .user, allowedAuthoritySigningKeys: [badCAKey]))
        { error in
            XCTAssertEqual((error as? NIOSSHError)?.type, .invalidCertificate)
        }
    }
    ...
    func testKeysIgnoreUnsuitableCAs() throws {
        ...
        let edKey = try NIOSSHCertifiedPublicKey(NIOSSHPublicKey(openSSHPublicKey: Fixtures.ed25519User))!
        XCTAssertNoThrow(
            try userKey.validate(principal: "foo", type: .user, allowedAuthoritySigningKeys: [badCAKey, caKey])
        )
        XCTAssertNoThrow(
            try hostKey.validate(
                principal: "localhost",
                type: .host,
                allowedAuthoritySigningKeys: [badCAKey, caKey],
                acceptableCriticalOptions: ["cats"]
            )
        )
        XCTAssertNoThrow(
    ...
    func testKeysContainingUnsupportedCriticalExtensionsAreRejected() throws {
        ...
        XCTAssertThrowsError(
            try hostKey.validate(
                ...
                type: .host,
                allowedAuthoritySigningKeys: [caKey],
                acceptableCriticalOptions: ["dogs"]
            )
        ) { error in
            XCTAssertEqual((error as? NIOSSHError)?.type, .invalidCertificate)
        }
    ...
    func testExtraCriticalExtensionsAreIgnored() throws {
        ...
        // Let's try both valid principals
        let criticalOptions = try hostKey.validate(
            principal: "localhost",
            type: .host,
            allowedAuthoritySigningKeys: [caKey],
            acceptableCriticalOptions: ["cats", "lizards"]
        )
        XCTAssertEqual(criticalOptions, ["cats": "dogs"])
    }
    ...
    func testKeysNotYetValidFailValidation() throws {
        ...
        // Let's try any random principal: this will be good for anything
        XCTAssertThrowsError(try userKey.validate(principal: "foo", type: .user, allowedAuthoritySigningKeys: [caKey]))
        { error in
            XCTAssertEqual((error as? NIOSSHError)?.type, .invalidCertificate)
        }
    }
    ...
    func testVerificationFailsWhenMutatingKey() throws {
        try self.testVerificationFailsOnMutation {
            $0.key = try NIOSSHPublicKey(openSSHPublicKey: Fixtures.ed25519UserBase)
        }
    }
    ...
    func testVerificationFailsWhenMutatingSignature() throws {
        try self.testVerificationFailsOnMutation {
            $0.signature = try NIOSSHPrivateKey(p256Key: .init()).sign(digest: SHA256.hash(data: [1, 2, 3, 4]))
        }
    }

    func testVerificationFailsWhenMutatingSignatureKey() throws {
        try self.testVerificationFailsOnMutation {
            $0.signatureKey = try NIOSSHPublicKey(openSSHPublicKey: Fixtures.ed25519UserBase)
        }
    }
    ...
    func testCoWBehavesAppropriatelyWhenMutatingKey() throws {
        try self.simpleCoWBehavesAppropriately {
            $0.key = try NIOSSHPublicKey(openSSHPublicKey: Fixtures.ed25519UserBase)
        }
    }
